...
import {
  Bell, Sun, BarChart3, Brain, Store, Camera,
  TrendingUp, TrendingDown, ChevronRight, Plus,
  Search, Heart, Leaf, Mic,
...
export const HomeScreen = ({ navigation }: any) => {
  ...
  const quickActions = isCooperative
    ? [
        { icon: Plus,      label: 'Add Listing', gradient: 'primary' as const, route: 'AddCrop' },
    ...
    >
      ...
      <Animated.View entering={FadeInDown.duration(500)} style={styles.header}>
        ...
        <TouchableOpacity
          ...
          style={[styles.bellWrap, { backgroundColor: colors.surfaceVariant }]}
        >
        ...
        <GradientCard>
          <View style={styles.weatherRow}>
            <View>
              ...
              <Text style={styles.weatherTip}>Great day for coffee harvesting ☕</Text>
            </View>
      ...
      {isCooperative && (
        <Animated.View entering={FadeInDown.delay(200).duration(500)} style={styles.statsRow}>
          {[
            ...
            >
              <Text style={styles.statEmoji}>{st.emoji}</Text>
              <Text style={[styles.statValue, { color: colors.text }]}>{st.value}</Text>
              <Text style={[styles.statLabel, { color: colors.textSecondary }]}>{st.label}</Text>
            </View>
          ))}
        </Animated.View>
      )}
          ...
          {quickActions.map((a) => (
            ...
            >
              <LinearGradient
                colors={
                  a.gradient === 'warm'
                    ? [colors.gradientWarmStart, colors.gradientWarmEnd]
      ...
      <Animated.View entering={FadeInDown.delay(400).duration(500)} style={{ paddingHorizontal: SPACING.lg }}>
        <TouchableOpacity onPress={() => navigation.navigate('Market')} activeOpacity={0.85}>
          ...
          >
            <View>
              <Text style={styles.bannerTag}>☕ COFFEE MARKET</Text>
      ...
      {/* ── AI Assistant banner ── */}
      <Animated.View entering={FadeInDown.delay(450).duration(500)} style={{ paddingHorizontal: SPACING.lg, marginTop: 12 }}>
        <TouchableOpacity
        ...
        >
          ...
          >
            <View>
              ...
              <View style={styles.bannerCta}>
                <Text style={styles.bannerCtaText}>Ask about coffee</Text>
                <ChevronRight size={14} color="#FFFFFF" />
              </View>
            </View>
            <Text style={{ fontSize: 80 }}>🤖</Text>
      ...
      <Animated.View entering={FadeInDown.delay(500).duration(500)} style={styles.section}>
        <View style={styles.sectionHeader}>
          <View style={styles.sectionTitleRow}>
            ...
            <Text style={[styles.sectionTitle, { color: colors.text }]}>Trending Prices</Text>
          </View>
        ...
        {loading ? (
          ...
          <Text style={{ color: colors.textSecondary, fontSize: 13, fontFamily: FONT.regular }}>
            No price data yet. Tap ML Update on Prices screen.
          </Text>
        ) : (
          trendingPrices.map((crop) => (
            <TouchableOpacity
              key={crop.id}
              onPress={() => navigation.navigate('PriceDetail', { cropId: String(crop.id) })}
              style={[styles.cropCard, { backgroundColor: colors.surface }, SHADOWS.sm]}
              activeOpacity={0.8}
            >
              <View style={[styles.cropEmoji, { backgroundColor: colors.surfaceVariant }]}>
                <Text style={{ fontSize: 22 }}>{crop.emoji}</Text>
              </View>
              <View style={styles.cropInfo}>
                <Text style={[styles.cropName, { color: colors.text }]}>{crop.name}</Text>
                <Text style={[styles.cropPrice, { color: colors.textSecondary }]}>
                  {crop.farmgate_rwf || crop.price} RWF/kg
                </Text>
              ...
              <View style={styles.cropTrend}>
                ...
                <Text style={[
                  styles.cropChange,
                  { color: crop.trend === 'rising' ? colors.success : colors.error },
                ]}>
      ...
      <Animated.View entering={FadeInDown.delay(600).duration(500)} style={styles.section}>
        <Text style={[styles.sectionTitle, { color: colors.text }]}>
          {isCooperative ? '☕ My Listings' : '☕ Available Coffee'}
        </Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {recentCrops.length === 0 ? (
            <Text style={{ color: colors.textSecondary, fontSize: 13, fontFamily: FONT.regular }}>
              {isCooperative ? 'No listings yet. Add your first one!' : 'No coffee available yet.'}
            </Text>
          ) : (
            recentCrops.map((item: any) => (
              <TouchableOpacity
                ...
                onPress={() =>
                  navigation.navigate(
                    isCooperative ? 'MyCrops' : 'CropDetail',
              ...
              >
                ...
                <Text style={[styles.recentSub, { color: colors.textSecondary }]}>
                  {item.quantity_display || `${item.quantity} ${item.unit}`}
                </Text>
                <Text style={{ fontSize: 11, fontFamily: FONT.semibold, color: colors.primary, marginTop: 4 }}>
                  {item.price} RWF
                </Text>
              </TouchableOpacity>
            ))
...
const styles = StyleSheet.create({
  ...
  actionLabel:     { fontSize: 11, fontFamily: FONT.medium, textAlign: 'center' },
  banner:          { borderRadius: RADIUS.xl, padding: 20, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
  bannerTag:       { fontSize: 11, fontFamily: FONT.semibold, color: 'rgba(255,255,255,0.85)', marginBottom: 8 },
  bannerCta:       { flexDirection: 'row', alignItems: 'center', gap: 4 },
  bannerCtaText:   { fontSize: 14, fontFamily: FONT.semibold, color: '#FFFFFF' },
