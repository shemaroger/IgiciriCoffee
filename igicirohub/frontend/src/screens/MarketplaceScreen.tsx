...
const COFFEE_FILTERS = [
  { label: 'All ☕',          value: 'all' },
  { label: 'Bourbon Arabica', value: 'bourbon arabica' },
  { label: 'Red Bourbon',     value: 'red bourbon' },
  { label: 'Yellow Bourbon',  value: 'yellow bourbon' },
  { label: 'Jackson',         value: 'jackson' },
  { label: 'Mibirizi',        value: 'mibirizi' },
...
export const MarketplaceScreen = ({ navigation }: any) => {
  ...
  const load = useCallback(async () => {
    ...
    if (selectedFilter !== 'all') params.append('search', selectedFilter);
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(400)} style={styles.headerSection}>
          <Text style={[styles.title, { color: colors.text }]}>☕ Coffee Market</Text>
          <Text style={{ fontSize: 12, fontFamily: FONT.regular, color: colors.textSecondary, marginBottom: 12 }}>
            6 varieties · Farm Gate · Cooperative · Export
          </Text>
          <View style={[styles.searchWrap, { backgroundColor: colors.surfaceVariant, borderColor: colors.border }]}>
            ...
            <TextInput
              ...
              placeholder="Search coffee listings..."
        ...
        >
          {COFFEE_FILTERS.map((f) => (
            <TouchableOpacity
              ...
              style={[styles.chip, {
                backgroundColor: selectedFilter === f.value ? colors.primary : colors.surfaceVariant,
              }]}
            >
        ...
        {loading ? (
          ...
          <View style={{ alignItems: 'center', marginTop: 60 }}>
            ...
            <Text style={{ color: colors.textSecondary, fontSize: 15, fontFamily: FONT.medium, marginTop: 12 }}>
              No coffee listings found
            </Text>
            <Text style={{ color: colors.textMuted, fontSize: 13, fontFamily: FONT.regular, marginTop: 4 }}>
              Only cooperatives can post listings
            </Text>
          </View>
        ) : (
          <View style={styles.listSection}>
            {crops.map((item, i) => (
              <Animated.View key={item.id} entering={FadeInDown.delay(i * 60).duration(400)}>
                <TouchableOpacity
                  onPress={() => navigation.navigate('CropDetail', { cropId: String(item.id) })}
                  activeOpacity={0.9}
                >
                  <View style={[styles.listingCard, { backgroundColor: colors.surface }, SHADOWS.sm]}>
                    <View style={styles.listingRow}>
                      ...
                      <View style={styles.listingInfo}>
                        ...
                        <Text style={[styles.listingQty, { color: colors.textSecondary }]}>
                          {item.quantity_display}
                        </Text>
                        <View style={styles.listingMeta}>
                          <MapPin size={12} color={colors.textMuted} />
                          <Text style={[styles.listingMetaText, { color: colors.textMuted }]}>
                      ...
                      <View style={styles.listingPriceCol}>
                        ...
                        <Text style={[styles.listingUnit, { color: colors.textSecondary }]}>
                          RWF/{item.unit}
                        </Text>
                      </View>
                    </View>
                    ...
                    <View style={styles.listingSeller}>
                      <Text style={[styles.sellerText, { color: colors.textSecondary }]}>
                        🤝 {item.farmer_name || 'Cooperative'}
                      </Text>
                      {item.description ? (
                        <Text
                          style={{ fontSize: 11, fontFamily: FONT.regular, color: colors.textMuted, marginTop: 2 }}
                          numberOfLines={1}
                        >
                          {item.description}
                        </Text>
                    ...
                    <View style={styles.btnRow}>
                      <TouchableOpacity
                        ...
                        onPress={() => handleCall(String(item.id))}
                      >
      ...
      <Modal visible={!!callingId} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={[styles.modalCard, { backgroundColor: colors.surface }]}>
            ...
            <Text style={[styles.modalText, { color: colors.text }]}>Calling seller...</Text>
          </View>
        </View>
      </Modal>
    </View>
...
const styles = StyleSheet.create({
  ...
  title:           { fontSize: 22, fontFamily: FONT.bold, marginBottom: 4 },
  ...
  modalCard:       { borderRadius: RADIUS.xl, padding: 32, alignItems: 'center', gap: 12 },
